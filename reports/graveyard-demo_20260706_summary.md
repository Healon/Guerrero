...
> 追記 2026-07-06（第六輪）：尖刺修短＋模型交接規格。
> ① 三段尖刺 3磚→2磚（48→32px；跳距 ~65px 下容錯 17→33px），Lin 實玩回饋。
>   實跳驗證三段皆無傷跨越（淨空 48px）、尖刺傷害不變。規則寫入 SPEC §9：
>   尖刺寬度上限 2 磚。
> ② 交接規格三件套（Lin 之後改用 Opus/Sonnet 開發）：專案 CLAUDE.md（路由
>   中心＋十條鐵則，session 自動載入）、docs/SPEC.md（架構契約：registry 表、
>   碰撞矩陣、傷害去重契約、上限掃描、時間鐵則、關卡格式與可達性硬數字、
>   重啟衛生、部署管線）、docs/PLAYBOOKS.md（五種改版劇本＋loop.step 測試
>   協議全文＋staging 陷阱清單＋最小回歸清單）。文件以 fresh-context Sonnet
>   盲測可依循性（只讀文件、禁讀原始碼，答「加投擲型新敵人」路線圖）：
>   檔案清單與順序、群組歸屬、契約引用、回歸清單全部正確；同時誠實揭露
>   7 個不確定點，其中 4 個為真缺口並已補進文件——①敵方傷害玩家側不開
>   去重欄位（無敵幀即防重複）②新投射物須明文宣告碰撞矩陣行（boss 鐮刀
>   不滅是刻意例外非通則）③站定型敵人先例與群組歸屬④enemies 資料第三種
>   格式。抽測→補洞→閉環，文件對 Sonnet 級模型可依循性已驗證。

> 追記 2026-07-06（第五輪）：第二武器升級 SOUL FIRE（亡靈追蹤火）。
> 每揮刀放出追蹤 220px 內最近敵人的綠火（穿牆、1 傷害、命中即滅、無目標 1.8s
> 熄滅）。與刀氣完全獨立（registry soulFire；任意順序拾取）。拾取點在 t=161
> 新增兩段木板跳台頂（幽靈/食屍鬼看守），真實跳躍可達性已實測。
> 關鍵設計：敵人與 boss 新增獨立 lastFire 欄位——若共用 lastSwing，火球會
> 蓋掉近戰的去重紀錄、讓同揮刀氣重複計傷；分離後實測 boss 貼臉單揮恰 -2
> （melee+fire）非 -3。調參一輪：初版實際觸及半徑(~130)與索敵半徑(220)不一致
> 造成「追一半熄滅」，speed 150→165、life 1400→1800 後 220px 穩定到達。
> AC G1–G8 全過、F 系列（刀氣防雙倍）與 boss 重生回歸全過、console 零錯誤。
> 測試側再犯「站位在深淵上空」兩次（t18-20 坑），均為測試 artifact 非遊戲 bug。
