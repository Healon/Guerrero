...
## G. 第二武器升級：SOUL FIRE 亡靈追蹤火（2026-07-06 加入）

| # | 條件 | 驗證方式 | 結果 |
|---|------|----------|------|
| G1 | 未拾取前揮刀無火球（即使已有刀氣） | eval 無升級與僅刀氣兩態揮刀 | ✅ fires=0 兩態皆然 |
| G2 | 拾取後每揮刀 1 枚；追向 220px 內最近存活敵人，命中扣 1 後消失 | eval 遠距對食屍鬼 | ✅ hp 2→1、火球消失 |
| G3 | （2026-07-06 改版）前上拋物線出手：出生於玩家前方、上拋 ~26px 弧、過弧頂轉平飛直行；無目標飛到壽命盡頭熄滅；穿牆 | eval 逐幀軌跡：spawn x>player、全程 x 單調前進、弧頂、glide、盡頭消失 | ✅ 全數成立；滑翔中若目標進入索敵圈會順勢鎖定（湧現行為，符合設計） |
| G4a | 貼臉單揮＝近戰1＋火球1（hp2 食屍鬼一揮死） | eval 貼臉揮擊 | ✅ |
| G4b | boss 貼臉單揮恰 -2（melee＋fire；wave 因 swingId 去重不重複）——驗 lastFire 分離 | eval boss 固定 recover | ✅ 恰 -2（非 -3） |
| G5 | 跨死亡保留；新一輪重置；HUD 火焰圖示對應顯示 | eval 深淵死亡＋restart | ✅ |
| G6 | 併發有界（冷卻×壽命 → ≤6）；150px 外火球追 boss 可達 | eval 連揮 6 刀＋遠距 boss | ✅ 峰值 5、遠距 -2（wave+fire） |
| G7 | 兩升級獨立：先拿火→只出火；補拿刀氣→雙發 | eval restart 後反序拾取 | ✅ |
| G8 | 新跳台真實可達（非傳送）：地面→板1→板2→小跳取得 | eval 模擬按鍵實跳 | ✅ y 192→160→128→取得 |

調參紀錄：初版 speed150/life1400 在 150px 距離「追到一半熄滅」（實際觸及半徑 ~130 與索敵半徑 220 不一致）→ 調為 speed165/steer0.09/life1800 後 220px 內穩定到達。

## H. 尖刺調整＋交接規格（2026-07-06）

| # | 條件 | 驗證方式 | 結果 |
|---|------|----------|------|
| H1 | 三段尖刺 3磚→2磚後，助跑起跳皆可無傷跨越 | eval 模擬按鍵各段實跳（god 排除敵人干擾後純幾何） | ✅ 三段皆越過、跨越淨空 48px |
| H2 | 尖刺傷害不變 | 站入尖刺 | ✅ 扣 2＋上彈 |
| H3 | 交接文件可依循性：fresh-context Sonnet 只讀文件回答「加投擲型新敵人」改版路線 | 派獨立 agent 抽測，比對實際架構 | ✅ 路線圖全對（檔案順序/群組/契約/回歸）；其揭露的 4 個文件缺口已補（敵方傷害不開去重欄位、投射物矩陣宣告義務、站定型敵人先例、enemies 第三種格式） |
| H4 | 最小回歸清單 | R1 移動 54px／R2 跳高差 30px／R4 刀氣 -1／R5 雙升級 -2／R6 重生可見／R7 console 0 | ✅ 全過 |

## 驗證紀錄

全部 AC（A–H）以 `game.loop.step()` 逐幀模擬輸入實測，詳見 reports/graveyard-demo_20260706_summary.md。
