...
**武器升級（兩種，獨立並存，先拿哪個都行）**：
- **SPIRIT BLADE**（青色光暈，中段跳台頂）：每次揮刀射出一道刀氣（150px 射程、
  可破箱），同一揮的近戰與刀氣不會對同一目標重複計傷。參數在 `config.WEAPON`。
- **SOUL FIRE**（綠色光暈，後段柵欄區新跳台頂、幽靈看守）：每次揮刀額外放出
  一團亡靈火，自動追向 220px 內最近的敵人（含 boss），穿牆、命中 1 傷害。
  無目標時漂移 1.8 秒熄滅。參數在 `config.SOULFIRE`。

升級皆跨死亡保留，Game Over 重開才重置。兩者全收＝近戰＋刀氣＋追蹤火三重輸出。
